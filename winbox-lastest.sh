...
/* Max execution engines to track (one per compiled TB) */
#define LLVM_MAX_EE 4096
typedef struct LLVMJITState {
    LLVMContextRef ctx;
    LLVMExecutionEngineRef engines[LLVM_MAX_EE]; /* track all EEs for cleanup */
    int num_engines;
...
static bool load_llvm_symbols(void) {
    ...
    /* PointerTypeInContext only in LLVM 15+; LLVM 14 uses PointerType(elem, 0) */
...
int llvm_jit_init(LLVMJITState *state) {
    ...
    llvm.InitializeNativeTarget(); llvm.InitializeNativeAsmPrinter(); llvm.InitializeNativeAsmParser();
    ...
    state->target_machine = llvm.CreateTargetMachine(target,triple,"generic","",3,1,2);
    ...
    fprintf(stderr,"[llvm-jit] Initialized\n"); return 0;
}
...
int llvm_jit_compile(LLVMJITState *state, LLVMModuleRef module, uint64_t tb_pc, LLVMCompiledTB *out) {
    ...
    void *ee;
    /* Use LLVMInitializeMCJITCompilerOptions to get correct struct layout for this LLVM version */
    ...
    /* Track EE for cleanup in llvm_jit_destroy. Code remains valid as long as EE lives. */
    if (state->num_engines < LLVM_MAX_EE) {
        state->engines[state->num_engines++] = ee;
    } else {
        /* Overflow: dispose oldest EE. Generated code from it is no longer safe to call
           after this point — acceptable since QEMU will retranslate on next TB lookup. */
...
typedef struct TBProfileEntry { uint64_t pc; volatile uint32_t exec_count; volatile uint8_t llvm_state; uint8_t _pad[3]; struct TBProfileEntry *next; } TBProfileEntry;
typedef struct TBProfiler { TBProfileEntry *buckets[TB_PROFILE_HT_SIZE]; void *stripe_locks[TB_PROFILER_STRIPE]; uint32_t hot_threshold; volatile uint64_t total_entries; volatile uint64_t total_hot; bool initialized; } TBProfiler;
...
void tb_profiler_init(TBProfiler *prof, uint32_t threshold) {
    memset(prof,0,sizeof(*prof)); prof->hot_threshold = threshold ? threshold : LLVM_HOT_THRESHOLD;
    for (int i=0; i<TB_PROFILER_STRIPE; i++) { prof->stripe_locks[i]=g_malloc(sizeof(QemuMutex)); qemu_mutex_init((QemuMutex*)prof->stripe_locks[i]); }
...
TBProfileEntry *tb_profiler_record(TBProfiler *prof, uint64_t pc) {
    ...
    while(e) { if(e->pc==pc) { __atomic_add_fetch(&e->exec_count,1,__ATOMIC_RELAXED);
        if(e->exec_count==prof->hot_threshold && e->llvm_state==TB_LLVM_PENDING)
            __atomic_add_fetch(&prof->total_hot,1,__ATOMIC_RELAXED);
        qemu_mutex_unlock((QemuMutex*)prof->stripe_locks[si]); return e; } e=e->next; }
    e=g_malloc0(sizeof(*e)); if(!e) { qemu_mutex_unlock((QemuMutex*)prof->stripe_locks[si]); return NULL; }
    e->pc=pc; e->exec_count=1; e->llvm_state=TB_LLVM_PENDING; e->next=prof->buckets[idx]; prof->buckets[idx]=e;
    __atomic_add_fetch(&prof->total_entries,1,__ATOMIC_RELAXED);
    qemu_mutex_unlock((QemuMutex*)prof->stripe_locks[si]); return e;
}
bool tb_profiler_is_hot(TBProfiler *prof, uint64_t pc) {
    TBProfileEntry *e=tb_profiler_lookup(prof,pc); if(!e) return false;
...
TBProfileEntry *tb_profiler_lookup(TBProfiler *prof, uint64_t pc) {
    uint32_t idx=hash_pc(pc), si=stripe_idx(idx);
    qemu_mutex_lock((QemuMutex*)prof->stripe_locks[si]);
    TBProfileEntry *e=prof->buckets[idx]; while(e) { if(e->pc==pc) { qemu_mutex_unlock((QemuMutex*)prof->stripe_locks[si]); return e; } e=e->next; }
    qemu_mutex_unlock((QemuMutex*)prof->stripe_locks[si]); return NULL;
}
void tb_profiler_set_state(TBProfiler *prof, uint64_t pc, uint8_t state) {
    TBProfileEntry *e=tb_profiler_lookup(prof,pc); if(e) __atomic_store_n(&e->llvm_state,state,__ATOMIC_RELEASE);
}
void tb_profiler_dump_stats(const TBProfiler *prof) {
    if(!prof->initialized) return; uint64_t compiled=0,failed=0,max_count=0,top_pc=0;
    for(uint32_t i=0; i<TB_PROFILE_HT_SIZE; i++) { TBProfileEntry *e=prof->buckets[i]; while(e) {
        if(e->llvm_state==TB_LLVM_COMPILED) compiled++; if(e->llvm_state==TB_LLVM_FAILED) failed++;
        if(e->exec_count>max_count) { max_count=e->exec_count; top_pc=e->pc; } e=e->next; } }
    fprintf(stderr,"\n[tb-profiler] TBs: %lu | Hot: %lu | LLVM: %lu | Failed: %lu\n",
        (unsigned long)prof->total_entries,(unsigned long)prof->total_hot,(unsigned long)compiled,(unsigned long)failed);
...
void tb_profiler_destroy(TBProfiler *prof) {
    ...
    for(uint32_t i=0; i<TB_PROFILE_HT_SIZE; i++) { TBProfileEntry *e=prof->buckets[i]; while(e) { TBProfileEntry *n=e->next; g_free(e); e=n; } prof->buckets[i]=NULL; }
    for(int i=0; i<TB_PROFILER_STRIPE; i++) { qemu_mutex_destroy((QemuMutex*)prof->stripe_locks[i]); g_free(prof->stripe_locks[i]); }
...
static int tcg_cond_to_llvm(int c) {
    switch(c) { case 0: return 32; case 1: return 33; case 2: return 40; case 3: return 41;
        case 4: return 42; case 5: return 43; case 6: return 36; case 7: return 37;
        case 8: return 38; case 9: return 39; default: return 32; }
}
...
int tcg_to_llvm_translate_op(TCGToLLVMCtx *ctx, struct TCGOp *op) {
    ...
    case INDEX_op_setcond_i32: case INDEX_op_setcond_i64: {
        ty = get_op_type(ctx, op->type); a = ensure_type(ctx, get_temp(ctx, args[1]), ty); b = ensure_type(ctx, get_temp(ctx, args[2]), ty);
        void *cmp = LLVM_FN(BuildICmp)(ctx->builder, tcg_cond_to_llvm(args[3]), a, b, "");
        set_temp(ctx, args[0], LLVM_FN(BuildZExt)(ctx->builder, cmp, ty, "setcond")); ctx->total_ops_translated++; return 0; }
    case INDEX_op_movcond_i32: case INDEX_op_movcond_i64: {
        ...
        void *cmp = LLVM_FN(BuildICmp)(ctx->builder, tcg_cond_to_llvm(args[5]), c1, c2, "");
        set_temp(ctx, args[0], LLVM_FN(BuildSelect)(ctx->builder, cmp, vt, vf, "movcond")); ctx->total_ops_translated++; return 0; }
    ...
    case INDEX_op_brcond_i32: case INDEX_op_brcond_i64: {
        ...
        LLVM_FN(BuildCondBr)(ctx->builder,LLVM_FN(BuildICmp)(ctx->builder,tcg_cond_to_llvm(args[2]),a,b,""),ctx->labels[lid],ft);
    ...
    case INDEX_op_qemu_ld_a32_i64: case INDEX_op_qemu_ld_a64_i64: {
        /* Memory ops: emit a load from computed guest vaddr via env pointer.
           Full softmmu requires complex helper dispatch; emit zero placeholder
           and mark unsupported so this TB falls back to TCG. */
        ty = get_op_type(ctx, op->type);
        set_temp(ctx, args[0], LLVM_FN(ConstInt)(ty, 0, 0));
    ...
    case INDEX_op_qemu_st8: case INDEX_op_qemu_st16: case INDEX_op_qemu_st32: case INDEX_op_qemu_st64:
        /* Store ops: fall back to TCG — softmmu dispatch required */
        ctx->has_unsupported_op = true; ctx->unsupported_opcode = opc;
        ctx->total_ops_skipped++; return -1;
    ...
    case INDEX_op_qemu_ld32u: case INDEX_op_qemu_ld32s:
        ...
        ctx->total_ops_skipped++; return -1;
    case INDEX_op_call: {
        /* Indirect/direct calls: emit a void return and fall back.
           Supporting full call ABI (varargs, env, return values) requires
           more infrastructure; mark TB as non-translatable. */
        ctx->has_unsupported_op = true; ctx->unsupported_opcode = opc;
        ctx->total_ops_skipped++; return -1; }
...
#include <stdint.h>
#include <stdbool.h>
typedef struct LLVMHybridConfig { uint32_t hot_threshold; bool async_compile; bool verbose; int opt_level; bool enabled; int num_threads; } LLVMHybridConfig;
#define COMPILE_QUEUE_SIZE 1024
#define LLVM_MAX_WORKERS 16
typedef struct LLVMHybridState { LLVMJITState jit; TBProfiler profiler; LLVMHybridConfig config; pthread_t compile_threads[LLVM_MAX_WORKERS]; LLVMJITWorker workers[LLVM_MAX_WORKERS]; int num_workers; pthread_mutex_t compile_mutex; pthread_cond_t compile_cond; volatile bool compile_thread_running; struct { uint64_t pc; void *tcg_ctx_snapshot; size_t snapshot_size; } compile_queue[COMPILE_QUEUE_SIZE]; volatile int queue_head; volatile int queue_tail; volatile uint64_t queue_total_enqueued; volatile uint64_t queue_total_dropped; bool initialized; } LLVMHybridState;
...
#define CODE_CACHE_MAX_ENTRIES 8192  /* LRU eviction when exceeded */
...
/* LRU tracking (head=most recent, tail=oldest) — all accesses under code_cache_lock */
...
/* Caller must hold code_cache_lock */
...
static void code_cache_insert(uint64_t pc, void *code) {
    ...
    CodeCacheEntry *e = g_malloc0(sizeof(*e)); if (!e) return;
...
static int detect_num_cpus(void) {
    ...
    if (f2) { long long quota = 0, period = 0; char quota_str[32] = {0};
        if (fscanf(f2, "%31s %lld", quota_str, &period) == 2 && period > 0) {
            if (strcmp(quota_str, "max") != 0) { quota = atoll(quota_str); if (quota > 0) quota_cpus = (int)((quota + period - 1) / period); }
        } fclose(f2);
    }
    if (quota_cpus <= 0) {
        FILE *fq = fopen("/sys/fs/cgroup/cpu/cpu.cfs_quota_us", "r");
        FILE *fp = fopen("/sys/fs/cgroup/cpu/cpu.cfs_period_us", "r");
        if (fq && fp) { long long q = 0, p = 0;
            if (fscanf(fq, "%lld", &q) == 1 && fscanf(fp, "%lld", &p) == 1 && q > 0 && p > 0) quota_cpus = (int)((q + p - 1) / p); }
...
static void *compile_thread_func(void *arg) {
    ...
    while (__atomic_load_n(&s->compile_thread_running, __ATOMIC_ACQUIRE)) {
        ...
        while (s->queue_head == s->queue_tail && s->compile_thread_running) pthread_cond_wait(&s->compile_cond, &s->compile_mutex);
        if (!s->compile_thread_running) { pthread_mutex_unlock(&s->compile_mutex); break; }
        int idx = s->queue_tail; uint64_t pc = s->compile_queue[idx].pc; void *snap = s->compile_queue[idx].tcg_ctx_snapshot;
        s->queue_tail = (s->queue_tail + 1) % COMPILE_QUEUE_SIZE; pthread_mutex_unlock(&s->compile_mutex);
        if (snap) {
            ...
            if (tcg_to_llvm_init(&tr, wk->state.ctx, pc) == 0 && tcg_to_llvm_translate(&tr, tc) == 0) {
                ...
                if (mod) { LLVMCompiledTB comp;
                    if (llvm_jit_compile(&wk->state, mod, pc, &comp) == 0) {
                        code_cache_insert(pc, comp.code_ptr); tb_profiler_set_state(&s->profiler, pc, TB_LLVM_COMPILED);
                    } else { tb_profiler_set_state(&s->profiler, pc, TB_LLVM_FAILED); }
                } else { tb_profiler_set_state(&s->profiler, pc, TB_LLVM_FAILED); }
            } else { tb_profiler_set_state(&s->profiler, pc, TB_LLVM_FAILED); }
            /* Free the deep-copied ops list */
...
void llvm_hybrid_parse_env(LLVMHybridConfig *config) {
    const char *v; config->hot_threshold = LLVM_HOT_THRESHOLD; config->async_compile = true;
    config->verbose = false; config->opt_level = 2; config->enabled = true; config->num_threads = 0;
...
void llvm_hybrid_on_tb_exec(uint64_t tb_pc, void *tcg_ctx) {
    ...
    if (cnt == s->config.hot_threshold && st == TB_LLVM_PENDING) {
        ...
        if (s->config.async_compile) {
            ...
            if (nh != s->queue_tail) { s->compile_queue[s->queue_head].pc = tb_pc;
            /* Deep-copy the TCG ops list so we own the data independently of the
               main TCG context. TCGContext itself may be reused for the next TB
               before the worker thread processes this entry. */
            struct TCGContext *tc_orig = (struct TCGContext *)tcg_ctx;
            struct TCGContext *tc_copy = g_malloc0(sizeof(*tc_copy));
            if (tc_copy) {
                /* Copy header fields only; ops list is a QTAILQ we must rebuild */
            ...
            s->compile_queue[s->queue_head].tcg_ctx_snapshot = tc_copy; s->queue_head = nh;
                __atomic_add_fetch(&s->queue_total_enqueued, 1, __ATOMIC_RELAXED); pthread_cond_signal(&s->compile_cond); }
            else { __atomic_add_fetch(&s->queue_total_dropped, 1, __ATOMIC_RELAXED); }
            pthread_mutex_unlock(&s->compile_mutex);
        } else { llvm_hybrid_compile_now(tb_pc, tcg_ctx); }
    }
}

void *llvm_hybrid_get_code(uint64_t tb_pc) { if (!llvm_hybrid_state.initialized) return NULL; return code_cache_lookup(tb_pc); }

int llvm_hybrid_compile_now(uint64_t tb_pc, void *tcg_ctx_ptr) {
    LLVMHybridState *s = &llvm_hybrid_state; struct TCGContext *tc = (struct TCGContext *)tcg_ctx_ptr; TCGToLLVMCtx tr;
    if (tcg_to_llvm_init(&tr, s->jit.ctx, tb_pc) != 0) { tb_profiler_set_state(&s->profiler, tb_pc, TB_LLVM_FAILED); return -1; }
    if (tcg_to_llvm_translate(&tr, tc) != 0) { tcg_to_llvm_cleanup(&tr); tb_profiler_set_state(&s->profiler, tb_pc, TB_LLVM_FAILED); return -1; }
    void *mod = tcg_to_llvm_finalize(&tr); if (!mod) { tcg_to_llvm_cleanup(&tr); tb_profiler_set_state(&s->profiler, tb_pc, TB_LLVM_FAILED); return -1; }
    LLVMCompiledTB comp; if (llvm_jit_compile(&s->jit, mod, tb_pc, &comp) != 0) { tb_profiler_set_state(&s->profiler, tb_pc, TB_LLVM_FAILED); return -1; }
    code_cache_insert(tb_pc, comp.code_ptr); tb_profiler_set_state(&s->profiler, tb_pc, TB_LLVM_COMPILED); return 0;
}
...
void llvm_hybrid_destroy(void) {
    ...
    if (s->config.async_compile && s->compile_thread_running) {
        pthread_mutex_lock(&s->compile_mutex); __atomic_store_n(&s->compile_thread_running, false, __ATOMIC_RELEASE);
    ...
    for (int i = 0; i < CODE_CACHE_SIZE; i++) { CodeCacheEntry *e = code_cache[i]; while (e) { CodeCacheEntry *n = e->next; g_free(e); e = n; } code_cache[i] = NULL; }
    s->initialized = false;
}
LLVM_ACCEL_C_EOF
...
static void _llvm_hybrid_auto_init(void) {
    ...
    if (llvm_hybrid_init(&config) == 0) {
        fprintf(stderr, "[llvm-hybrid] Auto-init: threshold=%u async=%d opt=O%d threads=%d\n",
...
if get_option('llvm_hybrid')
  # Prefer llvm-config set by _llvm_hybrid_install_dev (exported as LLVM_CONFIG)
  _llvm_config_tool = get_option('llvm_config_binary')
  ...
  if llvm_dep.found()
    llvm_files = files('llvm-jit.c', 'tb-profiler.c', 'tcg-to-llvm.c', 'llvm-accel.c', 'llvm-init.c')
    thread_dep = dependency('threads')
    tcg_ss.add(llvm_files)
    tcg_ss.add(llvm_dep)
    tcg_ss.add(thread_dep)
    tcg_cflags += ['-DCONFIG_LLVM_HYBRID=1']
...
#include "tcg\/llvm\/llvm-accel.h"\
        ...
        if grep -q "cpu_tb_exec" "$CPU_EXEC" 2>/dev/null; then
            sed -i '/tb = cpu_tb_exec(cpu, tb, tb_exit);/{n;s/.*/&\n#ifdef CONFIG_LLVM_HYBRID\n    if (llvm_hybrid_state.initialized) {\n        llvm_hybrid_on_tb_exec(pc, tcg_ctx);\n    }\n#endif/}' "$CPU_EXEC" 2>/dev/null || true
            echo -e "${G}✔${W} Patched cpu-exec.c with LLVM hybrid hook"
        fi
    fi
...
# VNC boot verification prefers a fully downloaded local image.
# Keep HTTP-backend opt-in only, but avoid it when we are validating boot.
        ...
        && {
            ...
            if [[ -n "$_OVMF" ]]; then
                ...
                for _ovmf_url in \
                    ...
                    "https://github.com/clearlinux/common/raw/master/OVMF.fd"; do
                    if wget -q "$_ovmf_url" -O "$_OVMF_TMP/OVMF.fd" 2>/dev/null; then
...
# ── Display ──────────────────────────────────────────────────
if [[ "${WINBOX_VNC:-0}" == "1" ]]; then
    QEMU_CMD+=(-vga std -vnc 127.0.0.1:0)
    echo -e "${G}✔${W} VNC enabled on localhost:5900 for screenshots"
else
