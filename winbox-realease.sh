...
# ── LLVM auto-detect: mặc định BẬT khi root, TẮT khi non-root ──
if [[ "$LLVM_NO" == "1" ]]; then
    LLVM_ACCEL=0
...
static int detect_num_cpus(void) {
    ...
    if (n < 2) return 2;
    if (n > LLVM_MAX_WORKERS) return LLVM_MAX_WORKERS;
    return (int)n;
}
...
if [[ "$LLVM_BUILD_OK" == "1" ]]; then
    ...
    _llvm_nthreads=${QEMU_LLVM_THREADS:-$(nproc 2>/dev/null || echo 2)}
...
PY
        ...
        if [[ "$use_rdp" == "y" && -x "$BORE_BIN" ]]; then
            ...
            # ── 5. Chờ lấy public endpoint (max 45s) ─────────────────
            # bore log: "listening at bore.pub:NNNNN" hoặc "remote_port=NNNNN"
            _BORE_RELAY_HOST="${BORE_RELAY%%:*}"
            echo -ne "${B}◜${W} Chờ bore endpoint"
            for i in $(seq 1 45); do
                ENDPOINT=""
                # Pattern 1: "listening at HOST:PORT"
                ENDPOINT=$(grep -m1 -oE "listening at [^ ]+:[0-9]+" "$BORE_LOG" 2>/dev/null \
                    | grep -oE '[^ ]+:[0-9]+$' | tail -1 || true)
                # Pattern 2: "remote_port=NNNNN" hoặc số port riêng lẻ
                if [[ -z "$ENDPOINT" ]]; then
                    _PORT=$(grep -m1 -oE "remote_port[= ]+[0-9]+|\bport [0-9]{4,5}\b" "$BORE_LOG" 2>/dev/null \
                        | grep -oE '[0-9]{4,5}' | tail -1 || true)
                    [[ -n "$_PORT" ]] && ENDPOINT="${_BORE_RELAY_HOST}:${_PORT}"
                fi
                # Pattern 3: fallback HOST:PORT bất kỳ
                if [[ -z "$ENDPOINT" ]]; then
                    ENDPOINT=$(grep -m1 -oE '[a-zA-Z0-9._-]+\.[a-zA-Z]{2,}:[0-9]{4,5}' "$BORE_LOG" 2>/dev/null \
                        | tail -1 || true)
                fi
                # Validate port range 1024-65535
                if [[ -n "$ENDPOINT" ]]; then
                    _EPORT="${ENDPOINT##*:}"
                    if [[ "$_EPORT" =~ ^[0-9]+$ ]] && (( _EPORT >= 1024 && _EPORT <= 65535 )); then
                ...
                # Nếu bore process đã crash thì thoát sớm
                _BORE_CUR=$(cat "$BORE_PID_FILE" 2>/dev/null || echo "")
                if [[ -n "$_BORE_CUR" ]] && ! kill -0 "$_BORE_CUR" 2>/dev/null; then
                    echo -e "\r${R}✘${W}  bore process đã thoát sớm — xem $BORE_LOG"
                    break
                fi
                echo -ne "."
                sleep 1
            ...
            if [[ -z "$PUBLIC" ]]; then
                echo -e "\r${Y}⚠${W}  Không lấy được endpoint — xem $BORE_LOG"
                echo -e "${Y}   Bore log (10 dòng cuối):${W}"
                tail -10 "$BORE_LOG" 2>/dev/null | sed 's/^/   /' || true
            fi
            ...
            (
                ...
                while true; do
                    ...
                    if [[ "$BORE_ALIVE" -eq 0 ]]; then
                        ...
                        NEW_ENDPOINT=""
                        for _ in $(seq 1 30); do
                            NEW_ENDPOINT=$(grep -m1 -oE "listening at [^ ]+:[0-9]+" "$WATCH_LOG" 2>/dev/null | grep -oE '[^ ]+:[0-9]+$' | tail -1 || true)
                            if [[ -z "$NEW_ENDPOINT" ]]; then
                                _WP=$(grep -m1 -oE "remote_port[= ]+[0-9]+|port [0-9]{4,5}" "$WATCH_LOG" 2>/dev/null | grep -oE '[0-9]{4,5}' | tail -1 || true)
                                [[ -n "$_WP" ]] && NEW_ENDPOINT="${WATCH_RELAY%%:*}:${_WP}"
                            fi
                            if [[ -z "$NEW_ENDPOINT" ]]; then
                                NEW_ENDPOINT=$(grep -m1 -oE '[a-zA-Z0-9._-]+\.[a-zA-Z]{2,}:[0-9]{4,5}' "$WATCH_LOG" 2>/dev/null | tail -1 || true)
                            fi
                            [[ -n "$NEW_ENDPOINT" ]] && break
                            sleep 1
                        done
                        ...
                        CURRENT_ENDPOINT=$(grep -m1 -oE "listening at [^ ]+:[0-9]+" "$WATCH_LOG" 2>/dev/null | grep -oE '[^ ]+:[0-9]+$' | tail -1 || true)
                        if [[ -z "$CURRENT_ENDPOINT" ]]; then
                            _CP=$(grep -m1 -oE "remote_port[= ]+[0-9]+|port [0-9]{4,5}" "$WATCH_LOG" 2>/dev/null | grep -oE '[0-9]{4,5}' | tail -1 || true)
                            [[ -n "$_CP" ]] && CURRENT_ENDPOINT="${WATCH_RELAY%%:*}:${_CP}"
                        fi
