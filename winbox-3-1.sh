...
# ════════════════════════════════════════════════════════════════
#  LLVM CAPABILITY PROBE  (chỉ detect, không cài gì)
#  Kiểm tra môi trường có đủ điều kiện dùng LLVM Hybrid không:
#    1. LLVM_ACCEL phải được bật (=1) — tức là root hoặc --accel-llvm
#    2. Có llvm-config (bất kỳ phiên bản 14-18) hoạt động được
#    3. Có llvm-dev headers (kiểm tra llvm-c/Core.h)
#  Kết quả: LLVM_CAPABLE=1 nếu đủ điều kiện, =0 nếu không
# ════════════════════════════════════════════════════════════════
LLVM_CAPABLE=0

_probe_llvm_capable() {
    # Nếu LLVM bị tắt hoặc không được yêu cầu → không cần probe
    if [[ "$LLVM_ACCEL" != "1" ]]; then
        LLVM_CAPABLE=0
        return
    fi

    # Tìm llvm-config hoạt động được
    local _lc=""
    for _v in 16 15 14 17 18; do
        if command -v "llvm-config-${_v}" &>/dev/null \
           && "llvm-config-${_v}" --version &>/dev/null 2>&1; then
            _lc="llvm-config-${_v}"; break
        fi
    done
    if [[ -z "$_lc" ]] && command -v llvm-config &>/dev/null \
       && llvm-config --version &>/dev/null 2>&1; then
        _lc="llvm-config"
    fi
    ...
    # Kiểm tra llvm-c/Core.h (LLVM dev headers)
    local _inc_dir
    _inc_dir=$("$_lc" --includedir 2>/dev/null || echo "")
    if [[ -n "$_inc_dir" && -f "${_inc_dir}/llvm-c/Core.h" ]]; then
        LLVM_CAPABLE=1
    elif dpkg -s "llvm-dev" &>/dev/null 2>&1 \
         || dpkg -s "llvm-16-dev" &>/dev/null 2>&1 \
         || dpkg -s "llvm-15-dev" &>/dev/null 2>&1 \
         || dpkg -s "llvm-14-dev" &>/dev/null 2>&1; then
        # Header chưa cài nhưng gói đã có → coi là đủ điều kiện (sẽ cài khi build)
        LLVM_CAPABLE=1
    else
        LLVM_CAPABLE=0
    fi
...
echo -e "${C}⬡  WINBOX${W}"
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    echo -e "${C}⚡ Acceleration: ${G}KVM (hardware)${W}"
elif [[ "$LLVM_CAPABLE" == "1" ]]; then
    echo -e "${C}⚡ Acceleration: ${G}LLVM (Fast)${W}"
else
    echo -e "${C}⚡ Acceleration: ${Y}TCG (software)${W}"
fi
echo -e "${C}════════════════════════════════════${W}"
...
# ── LLVM Hybrid: set environment variables ────────────────────
if [[ "$LLVM_BUILD_OK" == "1" ]]; then
    export QEMU_LLVM_HYBRID=1
    [[ -n "$LLVM_THRESHOLD" ]] && export QEMU_LLVM_THRESHOLD="$LLVM_THRESHOLD"
    echo -e "${C}⬡  LLVM Hybrid Backend: ENABLED (auto-detected root mode)${W}"
    ...
    echo -e "${Y}⚠${W}  LLVM Hybrid: build không thành công — dùng TCG thuần"
fi
