...
_detect_kvm() {
    ...
    if [[ $CAN_USE_KVM -eq 0 ]]; then
        if [[ -r /dev/kvm && -w /dev/kvm ]]; then
            ...
            echo -e "${Y}⚠${W}  /dev/kvm không đủ quyền read+write — fallback TCG"
        fi
    fi
...
_iso_mode_run() {
    ...
    if [[ "$KVM_AVAILABLE" == "1" ]] && [[ -r /dev/kvm && -w /dev/kvm ]]; then
        _kvm_ok=1
        _kvm_flags=(-enable-kvm -accel kvm)
        _cpu_arg="-cpu host,+invtsc,hv_relaxed,hv_spinlocks=0x1fff,hv_vapic,hv_time,hv_vendor_id=whatever"
        _machine_arg="-machine type=q35,kernel_irqchip=on"
        ...
        echo -e "${G}✔${W}  KVM + Hyper-V enlightenments + cpu host"
    else
