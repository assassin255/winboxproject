...
if [[ "${choice:-}" != "n" ]] && _detect_existing_qemu; then
...
# io_uring: probe thực tế bằng qemu-img để tránh "invalid aio option" khi QEMU build thiếu
_DISK_AIO="threads"
_qemu_bin_probe="${QEMU_BIN:-qemu-system-x86_64}"
if [[ -x "$_qemu_bin_probe" ]] || command -v "$_qemu_bin_probe" &>/dev/null; then
    if "$_qemu_bin_probe" -drive file=/dev/null,if=none,id=x,aio=io_uring,format=raw \
        -machine none -nographic 2>&1 | grep -qv "invalid aio"; then
        _DISK_AIO="io_uring"
    fi 2>/dev/null || true
fi
# Fallback: thử qemu-img --help nếu probe trên fail
if [[ "$_DISK_AIO" == "threads" ]] && command -v qemu-img &>/dev/null; then
    qemu-img --help 2>&1 | grep -q "io_uring" && _DISK_AIO="io_uring" || true
fi
