...
" 2>/dev/null
    ...
    _ensure_aria2 || true  # không fatal — download sẽ fallback wget/curl
...
# ════════════════════════════════════════════════════════════════
#  ARIA2 — đảm bảo aria2c có sẵn (conda → apt → static binary)
# ════════════════════════════════════════════════════════════════
_ensure_aria2() {
    ...
    if command -v conda &>/dev/null; then
        ...
        conda install -y -q aria2 > /dev/null 2>&1 \
            ...
            && return 0
        spin_fail "aria2 conda thất bại — thử apt..."
    fi

    # ── Thử 2: apt / apt-get (nếu root hoặc sudo) ───────────────
    local _apt=""
    ...
    if [[ -n "$_apt" ]]; then
        ...
        spin_fail "apt không cài được aria2 — thử static binary..."
    fi

    # ── Thử 3: static musl binary ───────────────────────────────
...
_iso_mode_run() {
    ...
    if [[ -f "$_iso_dir/disk.qcow2" ]]; then
        local _exist_sz; _exist_sz=$(qemu-img info "$_iso_dir/disk.qcow2" 2>/dev/null | awk '/virtual size/{print $3$4}' || echo "?")
        read -rp "$(echo -e "${Y}?${W}  disk.qcow2 đã có (${_exist_sz}) — tạo lại không? [y/N]: ")" _yn
        if [[ "${_yn,,}" == "y" ]]; then
            ...
            rm -f "$_iso_dir/disk.qcow2"
            spin_start "Tạo lại disk.qcow2 (${_disk_gb}G)..."
            qemu-img create -f qcow2 "$_iso_dir/disk.qcow2" "${_disk_gb}G" >/dev/null 2>&1
            spin_stop "Disk ${_disk_gb}G tạo xong"
        else
            echo -e "${G}✔${W}  Dùng disk cũ: $_iso_dir/disk.qcow2 (${_exist_sz})"
        fi
    else
    ...
    if [[ -n "$_cores_raw" && "$_cores_raw" -ge 1 ]]; then
        if [[ "$_cores_raw" -gt "$_host_cores" ]]; then
            echo -e "${Y}⚠${W}  ${_cores_raw} vCPU > host (${_host_cores}) — sẽ chậm do overcommit"
        elif [[ "$_cores_raw" -gt 8 ]]; then
            echo -e "${Y}⚠${W}  ${_cores_raw} vCPU: KVM spinlock overhead có thể làm VM CHẬM HƠN so với 2-4 vCPU"
            echo -e "${B}ℹ${W}  Khuyến nghị: 2-4 vCPU cho desktop/interactive, chỉ tăng nếu workload thực sự multi-threaded"
        fi
    ...
    local _kvm_ok=0
    local _kvm_flags=()
    local _cpu_arg="-cpu qemu64,+ssse3,+sse4.1,+sse4.2,+x2apic"
    local _machine_arg="-machine type=q35"
    local _aio="threads"
    local _smt_threads=1

    if [[ -r /dev/kvm ]]; then
        _kvm_ok=1

        # Detect hyperthreading — nếu host có HT thì expose cho VM
        local _ht
        _ht=$(lscpu 2>/dev/null | awk '/^Thread\(s\) per core/{print $NF}')
        [[ "$_ht" -ge 2 ]] 2>/dev/null && _smt_threads=2

        # kernel_irqchip=split: mỗi vCPU thread riêng, IOApic emulate trong kernel
        # → giảm VM-exit đáng kể khi nhiều vCPU
        _machine_arg="-machine type=q35,kernel_irqchip=split"

        # Hyper-V enlightenments đầy đủ cho Windows KVM multi-vCPU
        local _hv_flags="hv_relaxed,hv_spinlocks=0x1fff,hv_vapic,hv_time"
        _hv_flags+=",hv_vendor_id=whatever"
        # hv_ipi + hv_stimer: giảm IPI overhead khi nhiều vCPU
        _hv_flags+=",hv_ipi,hv_stimer,hv_stimer_direct"
        # hv_frequencies: Windows dùng TSC frequency từ hypervisor thay vì CPUID
        _hv_flags+=",hv_frequencies"
        # hv_runtime: expose vCPU runtime counter cho Windows scheduler
        _hv_flags+=",hv_runtime"

        _kvm_flags=(-enable-kvm -accel kvm)
        _cpu_arg="-cpu host,+invtsc,+topoext,${_hv_flags}"

        # io_uring nhanh hơn threads nếu kernel >= 5.1
        local _kmaj _kmin
        _kmaj=$(uname -r | cut -d. -f1)
        _kmin=$(uname -r | cut -d. -f2 | tr -cd '0-9')
        if [[ "$_kmaj" -gt 5 ]] || [[ "$_kmaj" -eq 5 && "$_kmin" -ge 1 ]]; then
            _aio="io_uring"
        fi

        echo -e "${G}✔${W}  KVM multi-threaded vCPU — kernel_irqchip=split + Hyper-V IPI/stimer"
    else
        echo -e "${Y}⚠${W}  KVM không có — TCG mode (chậm hơn đáng kể)"
    fi
    ...
    local _launch_cmd=(
        ...
        "${_kvm_flags[@]}"
        $_cpu_arg
        -smp "${_cpu_cores},sockets=1,cores=${_cpu_cores},threads=${_smt_threads}"
        -m "${_ram_gb}G"
        -object iothread,id=io1
        -drive file="$_iso_dir/disk.qcow2",if=none,id=disk0,format=qcow2,cache=unsafe,aio=${_aio},discard=on
        -device virtio-blk-pci,drive=disk0,iothread=io1,num-queues=4,queue-size=256
        -device virtio-balloon
        -cdrom "$_iso_dir/win.iso"
    )
    # TCG fallback accel — chỉ khi không có KVM
    if [[ "$_kvm_ok" == "0" ]]; then
        _launch_cmd+=(-accel "tcg,thread=multi,tb-size=512")
    fi
    ...
    _launch_cmd+=(
        ...
        -netdev user,id=n0,hostfwd=tcp::3389-:3389,smb=/tmp
        -device virtio-net-pci,netdev=n0,rx_queue_size=256,tx_queue_size=256
