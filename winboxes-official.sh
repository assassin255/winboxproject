...
_iso_mode_run() {
    ...
    if [[ -r /dev/kvm ]]; then
        ...
        _tcg_tb_mb=$(( _host_ram_iso * 1024 * 13 / 100 ))
        [[ "$_tcg_tb_mb" -lt 4096   ]] && _tcg_tb_mb=4096
        [[ "$_tcg_tb_mb" -gt 10240 ]] && _tcg_tb_mb=10240
...
if [[ "$cfg_mode" == "1" ]]; then
    ...
    mem_auto_gb=$(awk "BEGIN{printf \"%d\", ($mem_total_gb*0.65)+0.5}")
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    TCG_TB_MB=$(( _host_ram_gb * 1024 * 13 / 100 ))
    [[ "$TCG_TB_MB" -lt 4096  ]] && TCG_TB_MB=4096
    [[ "$TCG_TB_MB" -gt 10240 ]] && TCG_TB_MB=10240
