...
_iso_mode_run() {
    ...
    if [[ -f "$_iso_dir/disk.qcow2" ]]; then
        ...
        if [[ "${_yn,,}" == "y" ]]; then
            read -rp "$(echo -e "${B}💾${W} Dung lượng disk mới (GB) [mặc định ${_disk_gb}]: ")" _disk_raw
            _disk_raw=$(printf '%s' "${_disk_raw}" | tr -cd '0-9')
            [[ -n "$_disk_raw" ]] && _disk_gb="$_disk_raw"
    ...
    # ── Detect KVM ───────────────────────────────────────────────
    local _kvm_ok=0
    local _kvm_accel="-accel tcg,thread=multi,tb-size=3097152"
    local _cpu_arg="-cpu qemu64"
    local _machine_arg="-machine type=q35"
    if [[ -r /dev/kvm ]]; then
        _kvm_ok=1
        _kvm_accel="-accel kvm"
        _cpu_arg="-cpu host"
        _machine_arg="-machine type=q35"
