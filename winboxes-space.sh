...
_iso_mode_run() {
    ...
    if [[ -r /dev/kvm ]]; then
        ...
        _tcg_tb_mb=$(( _host_ram_iso * 1024 * 15 / 100 ))
        [[ "$_tcg_tb_mb" -lt 4096   ]] && _tcg_tb_mb=4096
        [[ "$_tcg_tb_mb" -gt 32768 ]] && _tcg_tb_mb=32768
...
if [[ "${win_choice:-}" == "3" && "${PGO_MODE:-0}" == "0" && "${KVM_AVAILABLE:-0}" == "0" ]]; then
    ...
    if [[ "$_auto_pgo_ok" == "1" ]]; then
        ...
        if tar -xzf "$_AUTO_PGO_ARCHIVE" -C "$_AUTO_PGO_ROOT" >/dev/null 2>&1; then
            ...
            [[ "$AUTO_BUILD" != "yes" ]] && AUTO_BUILD="yes" && \
                echo -e "${B}ℹ${W}  PGO use phase: force rebuild QEMU với Win11 profile"
        else
...
# FIX: PGO use phase luôn phải rebuild QEMU với profile — không được skip
if [[ "${PGO_MODE:-0}" == "1" && "${PGO_PHASE:-}" == "use" && "$AUTO_BUILD" != "yes" ]]; then
    AUTO_BUILD="yes"
    echo -e "${B}ℹ${W}  PGO use phase: force rebuild QEMU với profile đã lưu"
fi
...
_tcg_tune_common() {
    # MALLOC_ARENA_MAX=2: TCG multi-thread JIT có ít lock contention hơn với 2 arenas
    # (=4 gây thêm overhead với workload translation-heavy)
    export MALLOC_ARENA_MAX=2
    export MALLOC_MMAP_THRESHOLD_=131072
    export MALLOC_TRIM_THRESHOLD_=131072
    export JIT_SERIALIZE_OBJECT=1
    echo -e "${G}✔${W} JIT env vars set (MALLOC_ARENA_MAX=2)"
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    TCG_TB_MB=$(( _host_ram_gb * 1024 * 15 / 100 ))
    [[ "$TCG_TB_MB" -lt 4096  ]] && TCG_TB_MB=4096
    [[ "$TCG_TB_MB" -gt 32768 ]] && TCG_TB_MB=32768
...
# io_uring: kiểm tra kernel support trước, sau đó probe QEMU
# AppImage trong container/JupyterHub thường bị seccomp block io_uring
_DISK_AIO="threads"

# Bước 1: kiểm tra kernel có io_uring không (/proc/sys hoặc syscall probe)
_io_uring_kernel=0
...
# EFAULT(14)=args bad but syscall exists, ENOSYS(38)=not supported
import ctypes.util
...
# Bước 2: chỉ probe QEMU nếu kernel support đã xác nhận
if [[ "$_io_uring_kernel" == "1" ]]; then
    ...
    if [[ -x "$_qemu_bin_probe" ]] || command -v "$_qemu_bin_probe" &>/dev/null; then
        ...
        if echo "$_probe_out" | grep -qv "invalid aio" && \
           ! echo "$_probe_out" | grep -qi "io_uring\|not support\|Operation not permitted\|seccomp"; then
            _DISK_AIO="io_uring"
        fi
    fi
fi
if [[ "$_DISK_AIO" == "io_uring" ]]; then
    echo -e "${G}✔${W}  Disk bus: ${B}virtio${W} + aio=${B}io_uring${W} (QEMU hỗ trợ)"
else
    echo -e "${G}✔${W}  Disk bus: ${B}virtio${W} + aio=threads (QEMU không có io_uring)"
fi
QEMU_CMD+=(
