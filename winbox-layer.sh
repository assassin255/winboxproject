...
_qemu_build_tuning() {
    ...
    QEMU_BASE_CFLAGS="-O3 -march=native -mtune=native -pipe -fno-plt -fno-semantic-interposition -fomit-frame-pointer -fstack-protector-strong -ffunction-sections -fdata-sections${_fast_math_flag}"
...
_iso_mode_run() {
    ...
    if [[ -r /dev/kvm ]]; then
        ...
        _tcg_tb_mb=$(( _host_ram_iso * 1024 * 12 / 100 ))
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    TCG_TB_MB=$(( _host_ram_gb * 1024 * 12 / 100 ))
...
if [[ "${WINBOX_VNC:-1}" == "1" ]]; then
    QEMU_CMD+=(-vga virtio -vnc :0,share=force-shared)
