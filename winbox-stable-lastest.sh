...
#  aria2: cài qua conda (primary), fallback wget static binary, fallback wget
#  Conda: CHỈ dùng cho aria2 + fallback khi source build thất bại
...
_ensure_aria2() {
    ...
    # ── Thử 1: conda ────────────────────────────────────────────
    if command -v conda &>/dev/null; then
        spin_start "Cài aria2 từ conda (primary)..."
